...
if opcion == "Inicio":
    ...
    c1, c2 = st.columns(2)
    with c1:
        nombre, apellido = st.text_input("Nombre (Opcional)"), st.text_input("Apellido (Opcional)")
    with c2:
        rut, comuna = st.text_input("RUT (Obligatorio)"), st.text_input("Comuna")
        ...
        if os.path.exists(ARCHIVO_CSV):
            ...
            if not df.empty:
                ...
                if st.button("MARCAR COMO SOLUCIONADO"):
                    ...
                    if not os.path.exists(ARCHIVO_HISTORIAL): fila.to_csv(ARCHIVO_HISTORIAL, index=False, encoding="utf-8")
                    else: pd.concat([pd.read_csv(ARCHIVO_HISTORIAL), fila], ignore_index=True).to_csv(ARCHIVO_HISTORIAL, index=False, encoding="utf-8")
                    df[df['RUT'] != seleccion].to_csv(ARCHIVO_CSV, index=False, encoding="utf-8")
                    st.success(f"Caso {seleccion} movido al Historial."); st.rerun()
    ...
    if st.text_input("Clave de Acceso", type="password") == CLAVE_ADMIN:
        ...
        else: st.info("Sin casos solucionados.")
